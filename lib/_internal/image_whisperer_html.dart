...
Future<CanvasElement> _loadImage(String url, {bool bypassCheck = false}) {
  ...
  Completer<CanvasElement> result = Completer();
  ...
  image.onError.listen((Event event) {
    result.completeError(ImageLoadError(event is ErrorEvent ? event : null));
  });
  image.src = url;
...
Future<Blob> _canvasToBlob(CanvasElement canvas, String mimeType,
  ...
  try {
    ...
    Uint8List bytes =
        byteList is Uint8List ? byteList : Uint8List.fromList(byteList);
    return Future.value(Blob([bytes], mimeType));
  }
}
...
class BlobImage extends BaseImage {
  ...
  FutureOr<CanvasImage> toCanvasImage() async {
    return CanvasImage(await _convertBlobToCanvas(blob), name: name);
  }
...
class CanvasImage extends BaseImage {
  ...
  @override
  FutureOr<BlobImage> toBlobImage(String mimeType, {int? quality}) async {
    return BlobImage(await _canvasToBlob(canvas, mimeType, quality: quality),
        name: name);
  }
...
Future<BaseImage> _rotateIfNeeded(BaseImage image) async {
  if (image is BlobImage) {
    return readExifFromBlob(image.blob!)
      ...
      return Future.sync(() => image.toCanvasImage())
        ...
        CanvasElement backingCanvas = CanvasElement(
          width: width,
          height: height,
        );
        ...
        context.restore();
        return CanvasImage(backingCanvas, name: image.name);
      });
    });
  } else {
    return image;
...
Future<CanvasElement> _scale(
  ...
  int sw = input.width!;
  int sh = input.height!;
  CanvasElement canvas = CanvasElement(width: sw, height: sh);
  ...
  Uint32List targetLine = Uint32List(4 * targetWidth);
  Uint32List sampleCount = Uint32List(targetWidth);
  int tyc = 0;
  int srcLine = 0;
  int trgLine = 0;
...
class ImageProcessingPipeline {
  Future<BaseImage> process(BaseImage image) {
    Future<BaseImage> result = Future.sync(() => image);
